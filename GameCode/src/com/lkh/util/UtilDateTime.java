...
public class UtilDateTime{
	...
	private static int firstDayWeek(int month){
		Calendar cal = Calendar.getInstance();
		Date d = UtilDateTime.toSqlDate(month+"", "1", cal.get(Calendar.YEAR)+"");		
		cal.setTime(d);
		return cal.get(Calendar.DAY_OF_WEEK)-1;
	}
	
	private static int getToday(){
		return Integer.parseInt(UtilDateTime.toDateString(new Date(), "dd"));
	}
